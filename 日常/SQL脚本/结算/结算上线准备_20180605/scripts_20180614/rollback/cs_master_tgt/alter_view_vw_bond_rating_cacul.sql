create view vw_bond_rating_cacul as
  WITH client AS (
      ...
      FROM (lkp_region a_1
        JOIN lkp_ratingcd_xw b_1 ON ((
          ((substr((a_1.region_nm) :: text, 1, 2) = substr((b_1.constant_nm) :: text, 1, 2)) AND
           (a_1.parent_cd IS NULL)) AND (b_1.constant_type = 6))))
      ...
      SELECT
        ...
        b_1.ratingcd_nm
      FROM (exposure a_1
        JOIN lkp_ratingcd_xw b_1
          ON ((((a_1.exposure) :: text = (b_1.constant_nm) :: text) AND (b_1.constant_type = 5))))
      WHERE (a_1.isdel = 0)
  )
  SELECT
    nextval('seq_bond_rating_cacul' :: regclass)                 AS bond_rating_cacul_sid,
    a.secinner_id,
    a.security_cd,
    a.security_nm,
  ...
  FROM (((((((((((((((((((bond_basicinfo a
    ...
    LEFT JOIN (SELECT
                 ...
                 row_number()
                 OVER (
                   PARTITION BY a_1.secinner_id
                   ORDER BY a_1.change_dt DESC ) AS rnk
               FROM (bond_opvolchg a_1
                 JOIN lkp_charcode b_1 ON ((((a_1.chg_type_id = b_1.constant_id) AND (b_1.constant_type = 45)) AND
                                            ((b_1.constant_nm) :: text = ANY
                                             (ARRAY [('债券偿付本金' :: character varying) :: text, ('回售' :: character varying) :: text, ('到期' :: character varying) :: text, ('赎回' :: character varying) :: text])))))
               WHERE
                 (((a_1.change_dt IS NOT NULL) AND (a_1.change_dt <= to_date('20170430' :: text, 'yyyymmdd' :: text)))
                  AND (a_1.isdel = 0))) chg ON (((a.secinner_id = chg.secinner_id) AND (chg.rnk = 1))))
    LEFT JOIN (SELECT
                 x.secinner_id,
                 x.party_id
               FROM (bond_party x
                 JOIN lkp_charcode y ON ((((x.party_type_id = y.constant_id) AND (y.constant_type = 209)) AND
                                          ((y.constant_nm) :: text = ANY
                                           (ARRAY [('债务人' :: character varying) :: text, ('联合债务人' :: character varying) :: text, ('发起人' :: character varying) :: text])))))) d
      ON ((a.secinner_id = d.secinner_id)))
    ...
    LEFT JOIN (SELECT
                 bond_warrantor.bond_warrantor_sid,
                 ...
                 row_number()
                 OVER (
                   PARTITION BY bond_warrantor.secinner_id, bond_warrantor.warrantor_nm
                   ORDER BY bond_warrantor.src_updt_dt DESC NULLS LAST,
                     bond_warrantor.notice_dt DESC NULLS LAST ) AS row_num
               FROM bond_warrantor
    ...
    LEFT JOIN lkp_charcode lkporgnature
      ON ((((bdissuer.org_nature_id = lkporgnature.constant_id) AND (lkporgnature.constant_type = 46)) AND
           (lkporgnature.isdel = 0))))
    ...
    LEFT JOIN (SELECT
               ...
               WHERE ((a_1.isdel = 0) AND (a_1.is_new = 1))) l
      ON (((COALESCE(d.party_id, bdissuer.company_id) = l.company_id) AND (l.client_id = t.client_id))))
    ...
    LEFT JOIN (SELECT
                 ...
                 row_number()
                 OVER (
                   PARTITION BY rating_record.company_id, rating_record.client_id
                   ORDER BY rating_record.factor_dt DESC, rating_record.rating_start_dt DESC ) AS row_num
               FROM rating_record
               WHERE ((rating_record.rating_type = 2) AND (rating_record.rating_st = 1))) p
      ON (((((c.warrantor_id) :: numeric = p.company_id) AND (p.row_num = 1)) AND (p.client_id = t.client_id))))
  WHERE (a.isdel = 0);
