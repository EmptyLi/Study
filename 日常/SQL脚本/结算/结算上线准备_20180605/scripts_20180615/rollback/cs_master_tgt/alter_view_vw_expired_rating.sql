create view vw_expired_rating as
  WITH balancesheet_latest AS (
      SELECT
      ...
      WHERE (((compy_balancesheet.isdel = 0) AND (compy_balancesheet.rpt_timetype_cd = 1)) AND
      ...
      SELECT
        ...
        row_number()
        OVER (
          PARTITION BY compy_incomestate.company_id
          ORDER BY
            CASE
      ...
      WHERE (((compy_incomestate.isdel = 0) AND (compy_incomestate.rpt_timetype_cd = 1)) AND
  ...
  FROM (rating_hist_latest a
    JOIN compy_basicinfo b ON ((a.company_id = (b.company_id) :: numeric)))
  WHERE ((a.row_num = 1) AND (a.creation_time < (now() - '1 year' :: interval)))
  UNION ALL
  SELECT
    a.company_id,
    b.company_nm,
  ...
  FROM (((rating_hist_latest a
    JOIN compy_basicinfo b ON ((a.company_id = (b.company_id) :: numeric)))
    JOIN balancesheet_latest c ON ((a.company_id = (c.company_id) :: numeric)))
    JOIN incomestate_latest d ON ((a.company_id = (d.company_id) :: numeric)))
  WHERE (
    (((((a.row_num = 1) AND (c.row_num = 1)) AND (d.row_num = 1)) AND (c.updt_dt < (now() - '10 days' :: interval))) AND
     (d.updt_dt < (now() - '10 days' :: interval))) AND (a.creation_time >= (now() - '1 year' :: interval)));
