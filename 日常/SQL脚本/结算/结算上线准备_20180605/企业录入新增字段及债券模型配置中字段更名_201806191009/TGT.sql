select * from compy_bondissuer;
create table ray_compy_bondissuer as select * from compy_bondissuer;
drop view vw_bond_rating_cacul;
drop table compy_bondissuer;
...
create view vw_bond_rating_cacul as
  WITH client AS (
      ...
      SELECT
        ...
        b.model_id
      FROM (lkp_region a_1
        JOIN tmp_lkp_ratingcd_xw b ON ((
          ((substr((a_1.region_nm) :: text, 1, 2) = substr((b.constant_nm) :: text, 1, 2)) AND (a_1.parent_cd IS NULL))
      ...
      SELECT
        ...
        b.model_id
      FROM (exposure a_1
        JOIN tmp_lkp_ratingcd_xw b
          ON ((((a_1.exposure_sid) :: text = (b.constant_nm) :: text) AND (b.constant_type = 5))))
      WHERE (a_1.isdel = 0)
  )
  SELECT
    row_number()
    OVER (
      ORDER BY NULL :: text ) AS bond_rating_cacul_sid,
    a.model_id,
    a.secinner_id,
    a.security_cd,
    a.security_nm,
  ...
  FROM (SELECT DISTINCT
          COALESCE(e.model_id, r2.model_id, l.model_id)                  AS model_id,
          a_1.secinner_id,
          a_1.security_cd,
          a_1.security_nm,
        ...
        FROM (((((((((bond_basicinfo a_1
          ...
          LEFT JOIN (SELECT
                       ...
                       row_number()
                       OVER (
                         PARTITION BY a_2.secinner_id
                         ORDER BY a_2.change_dt DESC ) AS rnk
                     FROM (bond_opvolchg a_2
                       JOIN lkp_charcode b ON ((((a_2.chg_type_id = b.constant_id) AND (b.constant_type = 45)) AND
                                                ((b.constant_nm) :: text = ANY
                                                 (ARRAY [('债券偿付本金' :: character varying) :: text, ('回售' :: character varying) :: text, ('到期' :: character varying) :: text, ('赎回' :: character varying) :: text])))))
                     WHERE ((((a_2.change_dt IS NOT NULL) AND (a_2.change_dt <= now())) AND (a_2.isdel = 0)) AND
                            (b.isdel = 0))) chg ON (((a_1.secinner_id = chg.secinner_id) AND (chg.rnk = 1))))
          ...
          LEFT JOIN (SELECT
                     ...
                     WHERE ((a_2.isdel = 0) AND (a_2.is_new = 1))) l
            ON (((COALESCE(d.issuer_id, bdissuer.company_id) = l.company_id) AND (l.client_id = t.client_id))))
        WHERE (a_1.isdel = 0)) a;
