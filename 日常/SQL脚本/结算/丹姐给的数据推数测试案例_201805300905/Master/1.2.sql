1.2 完整的财务数据（入模指标）触发参考评级变动
保证 企业2有2016年12月31日的参考(基础)评级，推送该企业 2017年12月31日完整的财务数据（入模指标）+2017年经营数据（入模指标）缺失并推送：
...
where company_id in (select company_id
                     ...
                     where company_nm = '中国铁建股份有限公司' and is_del = 0)
...
where rating_record_id in (select rating_record_id
                           ...
                           where company_id in (select company_id
                                                from compy_basicinfo
                                                where company_nm = '中国铁建股份有限公司' and is_del = 0) and
...
where to_char(factor_dt, 'yyyy-mm-dd') = '2017-12-31' and secinner_id in
                                                          (select secinner_id
                                                           ...
                                                           where company_id in (select company_id
                                                                                from compy_basicinfo
                                                                                where company_nm = '中国铁建股份有限公司' and
                                                                                      is_del = 0));
commit;
