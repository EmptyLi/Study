...
CREATE VIEW  if not exists vw_bond_rating_cacul AS WITH client AS (
...
CREATE VIEW  if not exists vw_bond_rating_cacul_pledge AS WITH tmp_lkp_ratingcd_xw AS (
         SELECT a_1.model_id,
            a_1.constant_nm,
            a_1.ratingcd_nm,
            a_1.constant_type,
...
CREATE VIEW if not exists vw_bond_rating_cacul_warrantor AS WITH client AS (
         SELECT DISTINCT client_basicinfo.client_id
           FROM client_basicinfo
        ), tmp_lkp_ratingcd_xw AS (
         SELECT a_1.model_id,
...
alter view vw_bond_rating_cacul_pledge rename to ray_vw_bond_rating_cacul_pledge;
alter view vw_bond_rating_cacul_warrantor rename to ray_vw_bond_rating_cacul_warrantor;
