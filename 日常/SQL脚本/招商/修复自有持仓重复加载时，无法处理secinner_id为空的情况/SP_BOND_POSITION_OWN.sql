CREATE OR REPLACE PROCEDURE SP_BOND_POSITION_OWN IS
  ...
  修 改 人：RayLee
  修改时间：2018-04-25 14:26:58.554000
  修改内容：对于无法关联到我司债券的代码也要全部展示,那么对应的处理删除已经存在的数据的逻辑需要修改为
            security_cd + end_dt
  */
...
BEGIN
  ...
  INSERT INTO TEMP_BOND_POSITION_OWN
    ...
    SELECT SEQ_BOND_POSITION_OWN.NEXTVAL AS BOND_POSITION_OWN_SID, --1. 自主持仓流水号
           ...
           ROW_NUMBER() OVER(PARTITION BY T.SECURITY_CD, T.DATA_DT, T.PORTFOLIO_CD ORDER BY T.UPDT_DT DESC,T.RECORD_SID DESC, CASE WHEN T1.SRC_CD = 'CSCS' THEN 1 ELSE 0 END DESC) AS RNK --16.  记录序列
  ...
  DELETE FROM BOND_POSITION_OWN T
   WHERE EXISTS (SELECT *
           ...
           WHERE A.Security_Cd = T.Security_Cd
